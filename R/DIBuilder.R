...
function(builder, name, size, align, dwarf_type) {
  .Call("R_DIBuilder_CreateBasicType", builder, name, as.integer(size), as.integer(align), as.integer(dwarf_type))
}

newDebugFunctionType = 
function(builder, args, cu)
{
  .Call("R_DIBuilder_CreateFunctionType", builder, as.vector(args), cu)
}

newDebugPointerType = 
function(builder,baseType, name) {
  .Call("R_DIBuilder_CreatePointerType", builder, baseType, name)
}
